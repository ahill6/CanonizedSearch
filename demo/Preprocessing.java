...
import gov.nasa.jpf.symbc.repairproject.Runner.Methods;
...
public class Preprocessing{
	...
	public List<Methods> getMethods(){
		...
		for (Method m : methods.keySet()){
			if (!m.getName().contains("main")){
				ret.add(new Runner().new Methods(m.getName(), m.getNumParams()));
			}
		}
		return ret;
	}
	...
	public void visitAST(String type) {
		/* walks the AST in order to get a list of methods, method signatures, method and class characteristics
		 */
	...
	private Object whatKind(String a){
		/* Helper for automatically generating main methods to use with SPF
		 */
	...
	public void runMethods(String dir){
		// make list of files in dir
		
		List<File> files = populateFileList(dir);
		...
		for (File f : files){
			...
			try {
				converted = readFileToString(f.getAbsolutePath());
				//converted.replaceAll("public class", "class");
				//converted.replace("class Tmp", "public class Tmp");
				//converted.replaceAll("^[\\s]*(package) [\\s|\\w|.]*;[\\s]*$","");
				visitAST(converted);
				
				converted = converted.replaceAll(className, "Tmp");
				
				
				converted = converted.replace("\\s*(public)\\s*(static)\\s*(void)\\s*(main)", "public void main2");
				
				// need to remove the last "}" and add a main method (assuming they haven't put extra non-public classses at bottom)
				converted.lastIndexOf('}');
				String tmpConv = converted.substring(0, converted.lastIndexOf('}'));
				
				tmpConv += generateMain(new ArrayList<Method>(methods.keySet()));
				tmpConv += "}";
				write(tmpConv, "/Users/ahill6/Documents/workspace/jpf-symbc/src/examples/Tmp.java");
				
				/*
				for (Method m : methods.keySet()){ 
					String str = methods.get(m);
					System.out.println(str);
					if (str.equals("main")){
						continue;
					}
					
					tmpFile = generateMain(m);
					
					// write this method to Tmp.
					tmpFile = writeMethod(str, tmpFile);
					
					
					for (String call : methodCalls.get(str)){
						if (methodBodies.keySet().contains(call) && ! call.equals(str)){
							tmpFile = writeMethod(call, tmpFile);
						}
					}
					
					
					tmpFile += "}";
					write(tmpFile, "/Users/ahill6/Documents/workspace/jpf-symbc/src/examples/Tmp.java");
	...
	public boolean reformatToTmpIntroClassJava(String filename){
		try {
			Runtime rt = Runtime.getRuntime();
			System.out.println(filename);
			System.exit(1);
			String cd = "cd "+ 'a';
			Process pr = rt.exec("cd -jar map.jar time.rel test.txt debug");
			pr = rt.exec("mvn compile");
			int t = pr.waitFor();
			...
			original = original.replace("public static void main", "public static void main2");
			original = original.replace(className, "Tmp");
			...
			//original += generateMain(ddddddd); 					// should only be one method that isn't main
			System.out.println("post generate main");
			original += "}";
			System.out.println("It's all done but the writing");

						
			write(original, "/Users/ahill6/Documents/workspace/jpf-symbc/src/examples/Tmp.java");
			return true;
	...
	public boolean reformatForIntroClassJava(String filename){
		try {
			...
			// ONLY NEED TO ADD MAIN, CONSIDER WHETHER METHOD TO CALL IS STATIC OR NOT, ADD METHOD CALL TO MAIN
			// TODO - Make this line actually work 
			original = original.replace("public static void main", "public static void oldMain");
	...
	public void callSPF(Methods m, String target, String classp){
		...
		System.out.println(m.getNumParams());
		System.out.println(target);
		System.out.println(classp);
		System.exit(1);
		try {
			Config conf = JPF.createConfig(new String[0]);
			conf.setProperty("target", target);  // get rid of ".java"
	...
	class GenericASTVisitor extends ASTVisitor {
		...
		public boolean visit(TypeDeclaration t){
			if (name == null && (!t.getName().toString().contains("Obj"))){ // TODO- remove second part when no longer dealing with introclassJava
		...
		public boolean visit(MethodDeclaration m){
			if (m.isConstructor()){
				if ((constructor == null && m.getName().toString().equalsIgnoreCase(name))){
			...
			methodDetails.put(new Method(m.getName().toString(), m.parameters().size(), param, returnType, start, declaration, statc, 
					constructor!=null? constructor :new Constructor(null), m.toString()), m.getName().toString());
			return super.visit(m);
		}
